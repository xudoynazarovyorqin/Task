...
class Paynet extends BaseGateway
{
    ...
    private function CheckTransaction()
    {
        ...
        $transactionState = ($transaction->state == Transaction::STATE_CANCELLED_AFTER_COMPLETE) ? 2 : 1;
    ...
    private function PerformTransaction()
    {
        if ($this->getTransactionBySystemTransactionId())
            return "<ns2:PerformTransactionResult xmlns:ns2=\"http://uws.provider.com/\">".
                "<errorMsg>Транзакция уже существует</errorMsg>".
                "<status>201</status>".
                "<timeStamp>".DataFormat::toDateTimeWithTimeZone(now())."</timeStamp>".
                "<providerTrnId>" . $this->request->params['transactionId'] . "</providerTrnId>".
        ...
        if (is_null($model)) {
            return  "<ns2:PerformTransactionResult xmlns:ns2=\"http://uws.provider.com/\">".
                "<errorMsg>Заявка не найдена</errorMsg>".
                "<status>302</status>".
                "<timeStamp>".DataFormat::toDateTimeWithTimeZone(now())."</timeStamp>".
        ...
        $transaction = Transaction::create([
            ...
            'amount'                => 1 * $this->request->params['pay_amount'],
            ...
            'detail'                => $detail,
            'transactionable_type'  => Application::TABLE_NAME,
    ...
    private function CancelTransaction(){
        ...
        if ($transaction == null || $transaction->state == Transaction::STATE_CANCELLED || $transaction->state == Transaction::STATE_CANCELLED_AFTER_COMPLETE)
        {
    ...
    private function GetStatement()
    {
        ...
        $transactions = Transaction::where('payment_system', PaymentSystem::PAYNET)
            ->where('state','<>',Transaction::STATE_CANCELLED_AFTER_COMPLETE)
    ...
    private function GetInformation(){
        ...
        if ($model) {
            $client_fio = ($model->client) ? $model->client->name : '';
            return  "<ns2:GetInformationResult xmlns:ns2=\"http://uws.provider.com/\">".
                ...
                "<paramValue>".$client_fio."</paramValue>".
                "</parameters>".
                "<parameters>".
                "<paramKey>pay_amount</paramKey>".
                "<paramValue>" . $this->request->params['pay_amount'] . "</paramValue>".
                "</parameters>".
