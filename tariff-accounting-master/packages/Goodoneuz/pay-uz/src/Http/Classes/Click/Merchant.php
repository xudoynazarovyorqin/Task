...
class Merchant{
    ...
    public function validateRequest($request){
        ...
        switch($request['action']){
            ...
            case Click::REQUEST_COMPLATE:
                ...
                break;
            case Click::REQUEST_GETINFO:
                $result = $this->validateGetInfoRequest($request);
                break;
        }
    ...
    public function validateGetInfoRequest($request)
    {
        $sign = md5($request['service_id'] . $this->config['secret_key'] .
            $request['merchant_trans_id'] . $request['action'] . $request['sign_time']);
        // echo $sign .'/'. $request['sign_string'];
        return $sign == $request['sign_string'];
        //return true;
    }
