<template>
    <el-row>
        <header id="el-drawer__title" class="el-drawer__header">
			<span> {{ $t('message.new') }} {{ $t('message.Distribution of costs') + ' ' + ($t('message.warehouse_products')) | lowerFirst }}</span>
			<el-button v-can="['distribution_costs.create']" type="success" size="small" class="mr-1" :loading="waiting"
				@click="submit(false)"> {{ $t('message.save') }}</el-button>
			<el-button v-can="['distribution_costs.create']" type="primary" size="small" class="mr-1" :loading="waiting"
        ...
        <el-drawer size="90%" :with-header="false" :append-to-body="true" :visible.sync="drawerIncludeTransactions" ref="drawerIncludeTransactions" @closed="drawerClosed('drawerIncludeTransactionsChild')"  @opened="drawerOpened('drawerIncludeTransactionsChild')">
            <transactions drawer="drawerIncludeTransactions" ref="drawerIncludeTransactionsChild"></transactions>
        </el-drawer>
    </el-row>
</template>
...
<script>
    ...
    export default {
           ...
           computed: {
               ...mapGetters({
                   last_id: 'distributionCosts/lastId',
               })
           },
           methods: {
                ...mapActions({
                    getLastId: 'distributionCosts/getLastId',
                    save: 'distributionCosts/store',
                }),
                afterOpen(){
