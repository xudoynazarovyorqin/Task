<template>
    <el-row>
        <header id="el-drawer__title" class="el-drawer__header">
            <span> {{ $t('message.edit') }} {{ $t('message.Distribution of costs') + ' ' + ($t('message.warehouse_products')) | lowerFirst }}</span>
                    ...
                    <el-col :span="6">
                        <el-form-item :label="$t('message.type_document')" size="small">
                            <el-select v-model="type_document" clearable :placeholder="$t('message.type_document')">
                                <el-option :label="$t('message.buy_ready_product')" value="buy_ready_product_lists"></el-option>
                                <el-option :label="$t('message.sale')" value="sale_products"></el-option>
                                <el-option :label="$t('message.assembly')" value="assembly_items"></el-option>
                            </el-select>
                        </el-form-item>
                    </el-col>
                    <el-col :span="6">
                        <el-button @click="loadProducts()" icon="el-icon-search" type="primary" size="small"> {{ $t('message.Search') }} </el-button>
                    </el-col>
                </el-form>
            </el-card>

            <el-card class="mt-2" v-loading="loadingProducts">
                <el-row>
                    <el-col :span="6">
                    ...
                    <el-col :span="4">
                        <span>{{ $t('message.Total amount products') }}: {{ totalProductsPrice | formatMoney(2) }}</span>
                    </el-col>
                </el-row>
                <el-table
                    :data="items"
                    ...
                    style="width: 100%" class="crm-el-table">
                        ...
                        <el-table-column
                            ...
                            <template slot-scope="item">
                                {{ item.row.product ? item.row.product.name : '' }}
                            </template>
                        </el-table-column>
                        <el-table-column
                            :label="$t('message.type_document')">
                            <template slot-scope="item">
                                <template v-if="item.row.warehouse_productable_type == 'buy_ready_product_lists'">
                                    {{ $t('message.buy_ready_product') }}
                                </template>
                                <template v-if="item.row.warehouse_productable_type == 'sale_products'">
                                    {{ $t('message.sale') }}
                                </template>
                                <template v-if="item.row.warehouse_productable_type == 'assembly_items'">
                                    {{ $t('message.assembly') }}
                                </template>
                            </template>
                        </el-table-column>
...
<script>
    ...
    import warehouse_product from '@/utils/mixins/models/distribution_cost/warehouse-product';
    import { mapGetters, mapActions } from 'vuex';

    export default {
           mixins:[form,drawer,warehouse_product],
           ...
           methods: {
                ...
                setProductsToItems(){
                    this.old_items.forEach(element => {
                        let item = {};
                        let warehouse_product = element.additional_priceable
                        item.id = element.additional_priceable_id;
                        item.product = (warehouse_product) ? warehouse_product.product : '';
                        item.currency = (warehouse_product) ? warehouse_product.currency : '';
                        item.warehouse_productable_type = (warehouse_product) ? warehouse_product.warehouse_productable_type : '';
                        item.remainder = (warehouse_product) ? warehouse_product.remainder : 0;
                        item.buy_price = (warehouse_product) ? warehouse_product.buy_price : 0;
                        item.rate = (warehouse_product) ? warehouse_product.rate : 0;
                        item.created_at = (warehouse_product) ? warehouse_product.created_at : '';
