<template>
    <div>
   		<header id="el-drawer__title" class="el-drawer__header">
			<span> {{ $t('message.edit') }} {{ $t('message.client') | lowerFirst }}</span>
			<el-button v-can="['clients.update']" type="success" size="small" class="mr-1" :loading="waiting"
				@click="submit(false)"> {{ $t('message.save') }}</el-button>
			<el-button v-can="['clients.update']" type="primary" size="small" class="mr-1" :loading="waiting"
        ...
        <el-main v-loading="loading">
            <el-form ref="form" status-icon :model="form" :rules="rules" :label-position="'right'" class="style__label" size="mini">
                <el-row :gutter="20">
                    ...
                    <el-col :span="8">
                        <!-- <types v-model="form.type_id" :type_id="form.type_id" size="mini"></types> -->
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapGetters({
                old_client_contact_persons: 'clients/client_contact_persons',
                old_client_checking_accounts: 'clients/client_checking_accounts',
            })
        },
        methods:{
            ...mapActions({
            ...
            load(){
                if (!this.loading && this.client) {
                    ...
                    .then(res => {
                        this.changeLoading();
                        this.form = this.getForm;
                        this.client_contact_persons = JSON.parse( JSON.stringify( this.old_client_contact_persons ));
                        this.client_checking_accounts = JSON.parse( JSON.stringify( this.old_client_checking_accounts ))
                    })
                    .catch(err => {
                        this.changeLoading();
                        this.$alert(err)
