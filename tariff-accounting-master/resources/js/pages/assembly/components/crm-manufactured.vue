<template>
  	<div>
		<header id="el-drawer__title" class="el-drawer__header">
			<span>{{ $t('message.ready_products') }}   | {{ $t('message.assembly') }} № {{ assembly.id }}</span>
			<el-button v-can="['assemblies.manufactured']" type="success" size="small" class="mr-1" :loading="loading" @click="submit(false)"> {{ $t('message.save') }}</el-button>
			<el-button v-can="['assemblies.manufactured']" type="primary" size="small" class="mr-1" :loading="loading" @click="submit(true)"> {{ $t('message.save_and_exit') }}</el-button>
	...
	export default {
		...
		computed: {
			...mapGetters({
				...
				old_manufactured_products: 'assembly/manufactured_products',
				assembly_items: 'assembly/assembly_items',
                currencies: 'currencies/inventory',
			}),
			items: function() {
				return this.assembly_items.filter((item) => { return (item.product != null && (item.quantity - item.ready > 0) && !_.find(this.manufactured_products, ['assembly_item_id',item.id]))})
			}
		},
	    methods: {
			...mapActions({
				loadManufacturedProducts: 'assembly/getManufacturedProducts',
				loadAssemblyItems: 'assembly/getAssemblyItems',
				manufacturedStore : 'assembly/manufacturedStore',
			}),
			afterOpen(){
			...
			load(){
				if (!this.loading && this.assembly) {
					...
					this.loadManufacturedProducts(filter)
			...
			appendProduct(){
    			...
    			const product = this.selectedAssemblyItem.product;
                manufactured_product.assembly_item_id = this.selectedAssemblyItem.id;
				manufactured_product.product = this.selectedAssemblyItem.product;
				manufactured_product.warehouse_type_id = product.warehouse_type ? product.warehouse_type.id : null;
                manufactured_product.warehouse_id = '';
                manufactured_product.not_produced = this.selectedAssemblyItem.not_produced;
                manufactured_product.quantity = this.selectedAssemblyItem.not_produced;
                manufactured_product.ready = this.selectedAssemblyItem.ready;
