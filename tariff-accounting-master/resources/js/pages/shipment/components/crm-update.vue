<template>
    <el-col :span="24">
        <header id="el-drawer__title" class="el-drawer__header">
            <span>
                {{ $t('message.edit') }} {{ $t('message.Shipment') | lowerFirst }}
            </span>
            <el-button v-can="['shipments.update']" type="primary" size="small" class="mr-1" :loading="waiting" @click="submit()"> {{ $t('message.save_and_exit') }}</el-button>
...
<script>
    ...
    export default {
        ...
        computed:{
            ...mapGetters({
                old_items: 'shipments/shipment_products'
            })
        },
        methods:{
            ...mapActions({
                save: 'shipments/update',
            ...
            load(){
                if (!this.loading && this.shipment) {
                    this.changeLoading(true);
                    this.edit(this.shipment.id)
                    .then(res => {
                        this.changeLoading(false);
                        this.form = this.getForm;
                        this.old_items.forEach(item => {
                            this.pushProduct(item.product,item.available,item.booked,item.quantity,item.issued_from_booked);
                        });                        
                    })
                    .catch(err => {
