<template>
    <el-col :span="24">
        <header id="el-drawer__title" class="el-drawer__header">
            <span>
                {{ $t('message.new') }} {{ $t('message.Shipment') | lowerFirst }}
            </span>
            <el-button v-can="['shipments.create']" type="primary" size="small" class="mr-1" :loading="waiting" @click="submit()"> {{ $t('message.save_and_exit') }}</el-button>
                ...
                <el-form ref="form" :model="form" :rules="rules" :label-position="'right'" label-width="150px">
                    <el-col>
                        <el-form-item size="small" prop="number" class="d-inline-flex crm-document-number">
                            ...
                            <template slot="label">
                                <span class="document-title">{{ $t('message.Shipment') }} {{ $t('message.n') }}</span>
                            </template>
                        </el-form-item>
                    ...
                    <el-col :span="8">
                        <shipmentable-types v-model="form.shipmentable_type" :shipmentable_type="form.shipmentable_type" :disabled="false"></shipmentable-types>
                    </el-col>
                    <el-col :span="8">
                        <el-form-item :label="columns.shipmentable_id.title" prop="shipmentable_id">
                            <el-input v-model="form.shipmentable_id" size="small"></el-input>
                        </el-form-item>
                    </el-col>
                </el-form>
            </el-card>
            <el-card class="mt-2 pb-2">
                <p>{{ $t('message.Before transferring the product, please choose why the product will be shipped (required document), the reason is to show the product saved for this document') }}</p>
                <el-table size="medium" :data="items" style="width: 100%" class="crm-el-table" v-loading="loadingProduct">
                    <template slot="empty">
                        <span></span>
                    </template>
...
<script>
    ...
    import shipment from "@/utils/mixins/models/shipment";
    import documents from "./include/crm-documents";

    export default {
        mixins:[drawer,shipment,form],
        ...
        computed:{
            ...mapGetters({
                lastId: 'shipments/lastId',
            })
        },
        methods:{
            ...mapActions({
                save: 'shipments/store',
                getLastId: 'shipments/getLastId',
                getReservations: 'shipments/getReservations'
            }),
            afterOpen() {
            ...
            afterLeave(){
                this.$store.commit("shipments/SET_SELECTED_ROW",null);
                this.items = [];
            }
        }
    }
