<template>
    <section class="invoice" v-loading="loading" id="print">
        <div class="row">
            <div class="col-12">
                <h2 class="page-header">
                    <i class="el-icon-goods"></i> {{ $t('message.buys')}}.
                    <small class="float-right">{{ $t('message.date')}}: {{ model.datetime }}</small>
                </h2>
            </div>
        </div>
        <div class="row invoice-info">
            ...
            <div class="col-sm-6 invoice-col">
                <b>{{ $t('message.buys')}} #{{model.id}}</b><br>
                <br>
        ...
        <div class="row">
            ...
            <div class="col-12 table-responsive">
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="(buy_material,index) in buy_materials" :key="index">
                            <td>{{ buy_material.material ? buy_material.material.name : '' }}</td>
                            <td>{{ buy_material.qty_weight | formatNumber }}</td>
                            <td>{{ buy_material.material ? buy_material.material.measurement ? buy_material.material.measurement.name : '' : '' }}</td>
                            <td>{{ buy_material.price | formatNumber }} {{ (buy_material.currency) ? buy_material.currency.symbol : '' }}</td>
                            <td>{{ (buy_material.qty_weight * buy_material.price) | formatNumber }} {{ (buy_material.currency) ? buy_material.currency.symbol : '' }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
