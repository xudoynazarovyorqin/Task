...
<div class="row table-sm mr-0 ml-0 p-0">
    <div class="crm-content-header d-flex w-100 mb-2">
        <div class="crm-content-header-left d-flex w-50">
            <div class="crm-content-header-left-item mr-3" style="width:200px">
                <h5 class="d-inline mr-2 font-weight-bold">{{ $t("message.buys") }}</h5>
        ...
        <div class="crm-content-header-right d-flex w-50 justify-content-end">
            <div class="crm-content-header-right-item">
                <el-button v-can="'buys.create'" size="mini" @click="drawerCreate = true" icon="el-icon-circle-plus-outline"> {{ $t('message.create') }} </el-button>
            </div>
            <div class="crm-content-header-right-item">
                <crm-edit-dropdown v-can="'buys.delete'" :items="selectedItems" @delete="multipleDelete">
                </crm-edit-dropdown>
            </div>
            <div class="crm-content-header-right-item">
            ...
            <!-- <div class="crm-content-header-right-item">
                <router-link :to="{name: 'buyNotifications.index'}">
                    <el-button size="mini">
    ...
    >
        ...
        <thead>
            <tr>
                ...
                <crm-th v-can="'buys.coming'" :sort="sort" :column="columns.success" @c-change="updateSort"></crm-th>
            ...
            <tr>
                ...
                <th v-if="columns.provider_id.show">
                    <providers size="mini" v-model="filterForm.provider_id"></providers>
                </th>
                <th v-if="columns.contract_provider_id.show">
                    <contracts :provider_id="this.filterForm.provider_id" v-model="filterForm.contract_provider_id"></contracts>
                </th>
                ...
                <th v-if="columns.date.show">
                    <el-date-picker v-model="filterForm.date" :placeholder="columns.date.title" size="mini">
                    </el-date-picker>
                </th>
                ...
                <th v-if="columns.not_paid.show">
                    <el-input size="mini" :placeholder="columns.not_paid.title" clearable></el-input>
                </th>
                <th v-if="columns.status_id.show">
                    <statues size="mini" v-model="filterForm.status_id"></statues>
                </th>
                ...
                <th v-can="'buys.coming'" v-if="columns.success.show"></th>
        ...
        <transition-group name="flip-list" tag="tbody">
            <tr v-for="buy in list" :key="buy.id" :class="{'table-active': (selectedItems.indexOf(buy) > -1)}" class="cursor-pointer" @dblclick="show(buy)" :title="$t('message.Double tap to view')">
                ...
                <td v-if="columns.materials.show">
                    <span @click="showItems(buy)" style="cursor: pointer; color: #3490dc;">
                        {{ $t('message.view')}}
                    </span>
                </td>
                ...
                <td v-if="columns.success.show" class="settings-td" v-can="'buys.coming'">
                    <el-button size="mini" round :type="receiveBtnType(buy)" @click="coming(buy)">{{$t('message.coming')}} ( {{ Math.ceil(100 - (100 * (buy.waiting_items_count / buy.items_count))) }}%)</el-button>
                </td>
                <td v-if="columns.settings.show" class="settings-td">
                    <crm-setting-dropdown :model="buy" name="buys" :actions="actions" @edit="edit" @print="print" @delete="destroy" @show="show">
        ...
        <table class="table" v-loading="loadingItems">
            ...
            <tbody>
                <tr v-for="(item ,index) in items" :key="index">
                    ...
                    <td>{{ item.qty_weight | formatNumber }} {{ item.material | addMeasurement(item.qty_weight) }}</td>
                    <td>{{ (item.qty_weight - item.not_enough) | formatNumber }} {{ item.material | addMeasurement(item.qty_weight - item.not_enough) }}</td>
                    <td>
                        {{ (item.material && item.material.measurement) ? item.material.measurement.name : '' }}
                    </td>
                    <td>{{ item.price | formatNumber }} {{ (item.currency) ? item.currency.symbol : '' }}</td>
                    <td>{{ (item.qty_weight * item.price) | formatNumber }} {{ (item.currency) ? item.currency.symbol : '' }}</td>
                </tr>
            </tbody>
        </table>
    </el-dialog>
    <el-dialog :title="$t('message.buy_detail')" :visible.sync="visibleShow" width="80%" top="5vh" @closed="empty()">
    ...
    <el-drawer :with-header="false" :visible.sync="drawerReceive" size="90%" ref="drawerReceive" @closed="drawerClosed('drawerReceiveChild')" @opened="drawerOpened('drawerReceiveChild')">
        <receive drawer="drawerReceive" :buy="selectedItem" :open="drawerReceive" ref="drawerReceiveChild"></receive>
    </el-drawer>
</div>
</template>
<script>
	...
	export default {
		mixins: [list],
		components: { CrmCreate, CrmUpdate, CrmShow, receive,
            providers,
            users,
            contracts,
            statues
		...
		computed: {
			...mapGetters({
				...
				pagination: "buys/pagination",
				filter: "buys/filter",
				sort: "buys/sort",
				users: "users/list"
			}),
			actions: function() {
				return ["edit", "print", "show", "delete"];
			},
			total: function() {
		...
		methods: {
			...mapActions({
				updateList: "buys/index",
				updateSort: "buys/updateSort",
				updateFilter: "buys/updateFilter",
				updateColumn: "buys/updateColumn",
				updatePagination: "buys/updatePagination",
				refreshData: "buys/refreshData",
				delete: "buys/destroy",
				empty: "buys/empty",
				multiDelete: "buys/multiDelete",
                printModel: "buys/print",
                loadItems: 'buys/items',
			}),
			fetchMessages() {
				this.$store.dispatch("buyNotifications/count")
					.then(res => {
