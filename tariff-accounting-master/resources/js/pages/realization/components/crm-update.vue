<template>
    <el-col :span="24">
        <header id="el-drawer__title" class="el-drawer__header">
            <span>
                {{ $t('message.edit') }} {{ $t('message.Realization') | lowerFirst }}
            </span>
            <el-button v-can="['realizations.update']" type="primary" size="small" class="mr-1" :loading="waiting" @click="submit()"> {{ $t('message.save_and_exit') }}</el-button>
    ...
    export default {
        ...
        computed:{
            ...mapGetters({
                ...
                old_items: 'realizations/realization_materials'
            })
        },
        methods:{
            ...mapActions({
                save: 'realizations/update',
                getLastId: 'realizations/getLastId',
                loadReservations: 'realizations/loadReservations',
                edit: 'realizations/show',
            }),
            afterOpen() {
            ...
            load(){
                if (!this.loading && this.realization) {
                    this.changeLoading(true);
                    this.edit({id: this.realization.id})
                    .then(res => {
                        this.changeLoading(false);
                        this.form = this.getForm;
                        this.old_items.forEach(item => {
                            this.pushMaterial(item.material,item.available,item.booked,item.quantity,item.issued_from_booked);
                        });                        
                    })
                    .catch(err =>{
                        this.$alert(err);
