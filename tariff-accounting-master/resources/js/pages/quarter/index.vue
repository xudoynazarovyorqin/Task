<template>
  <div class="app-container">
    <div class="mytabel table-sm mr-0 ml-0 p-0">
      <div class="row table-sm mr-0 ml-0 p-0 mb-3 width-100">
        <div class="col-7 p-0 align-self-center font-weight-bold d-flex align-items-center">
          <h5 class="d-inline mr-2 font-weight-bold">{{ $t("message.quarters") }}</h5>
        ...
        >
          ...
          <div class="crm-content-header-right-item mr-2">
              <export-excel :data="list" :fields="excel_fields" @fetch="controlExcelData()" :worksheet="$t('message.quarters')" :name="$t('message.quarters')">
                  <el-button size="mini">
      ...
      <table class="table table-bordered table-hover mr-0 ml-0 p-0 bg-white" v-loading="loadingData" :element-loading-text="$t('message.loading')" element-loading-spinner="el-icon-loading">
        ...
        <transition-group name="flip-list" tag="tbody">
            <tr v-for="quarter in list" :key="quarter.id" class="cursor-pointer" @dblclick="edit(quarter)" :title="$t('message.Double tap to edit')">
                <td v-if="columns.id.show"> {{ quarter.id }} </td>
                <td v-if="columns.name.show"> {{ quarter.name | truncate }} </td>
                <td v-if="columns.description.show"> {{ quarter.description }}</td>
                <td v-if="columns.created_at.show">{{ quarter.created_at | dateFormat }}</td>
                <td v-if="columns.updated_at.show">{{ quarter.updated_at | dateFormat }}</td>
                <td v-if="columns.settings.show" class="settings-td">
                    <crm-setting-dropdown :model="quarter" name="quarters" :actions="actions" @edit="edit" @delete="destroy"></crm-setting-dropdown>
                </td>
            </tr>
        </transition-group>
      </table>
    ...
    <el-drawer :with-header="false" :visible.sync="drawerUpdate" size="80%" ref="drawerUpdate" @closed="drawerClosed('drawerUpdateChild')" @opened="drawerOpened('drawerUpdateChild')">
        <crm-update drawer="drawerUpdate" :open="drawerUpdate" :quarter="selectedItem" ref="drawerUpdateChild"></crm-update>
    </el-drawer>
  </div>
</template>
<script>
...
export default {
  ...
  computed:{
      ...mapGetters({
          ...
          pagination: "quarters/pagination",
          filter: "quarters/filter",
          sort: "quarters/sort",
      }),
      actions: function(){
          return ['edit','delete'];
  ...
  methods: {
      ...mapActions({
              updateList: 'quarters/index',
              updateSort: "quarters/updateSort",
              updateFilter: "quarters/updateFilter",
              updateColumn: "quarters/updateColumn",
              updatePagination: "quarters/updatePagination",
              delete: 'quarters/destroy',
              refreshData: 'quarters/refreshData',
      }),
      controlExcelData(){
          this.excel_fields = {};
