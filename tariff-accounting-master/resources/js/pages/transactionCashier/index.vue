<template>
  <div class="app-container">
    <div class="mytabel table-sm mr-0 ml-0 p-0">
      <div class="row table-sm mr-0 ml-0 p-0 mb-3 width-100">
        <div class="col-7 p-0 align-self-center font-weight-bold d-flex align-items-center">
          <h5 class="d-inline mr-2 font-weight-bold">Транзакции</h5>
      ...
      >
        <thead>
          <tr>
            ...
            <th v-if="columns.commend.show">{{ columns.commend.title }}</th>
            <th v-if="columns.data.show">{{ columns.data.title }}</th>
            <th v-if="columns.data2.show">{{ columns.data2.title }}</th>
            <th v-if="columns.changed.show">{{ columns.changed.title }}</th>
            <th v-if="columns.return.show">{{ columns.return.title }}</th>
          ...
          <tr>
            ...
            <th v-if="columns.commend.show">
              <el-input
                clearable
                size="mini"
                v-model="filterForm.commend"
                :placeholder="columns.commend.title"
              ></el-input>
            </th>
            ...
            <th v-if="columns.changed.show">
              <el-date-picker
                clearable
                size="mini"
                type="date"
                v-model="filterForm.changed"
                :placeholder="columns.changed.title"
              ></el-date-picker>
            </th>
        ...
        <transition-group name="flip-list" tag="tbody">
          <tr v-for="user in list" :key="user.id" class="cursor-pointer">
            ...
            <td v-if="columns.commend.show">{{ user.commend }}</td>
            <td v-if="columns.data.show">{{ user.data }}</td>
            <td v-if="columns.data2.show">{{ user.data2 }}</td>
            <td v-if="columns.changed.show">{{ user.changed }}</td>
    ...
    <el-dialog title="Возврат Транзакции" :visible.sync="dialogTableVisible">
      <el-form ref="form" :model="form" label-width="120px" size="medium">
        <el-form-item label="Тип Возврат">
          <el-select v-model="form.region" class="w-100">
            <el-option label="Zone one" value="shanghai"></el-option>
            <el-option label="Zone two" value="beijing"></el-option>
          </el-select>
        </el-form-item>
      ...
      list: [
        {
          ...
          rate: "1 ",
          commend: "Канстовар",
          data: "22.05.2020 13:04",
          data2: "22.05.2020 13:04",
          changed: "22.05.2020 13:04",
          return: ""
        },
        {
          id: "1",
