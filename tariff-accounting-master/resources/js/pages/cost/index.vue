<template>
    <div class="row table-sm mr-0 ml-0 p-0">
        <div class="crm-content-header d-flex w-100 mb-2">
            <div class="crm-content-header-left d-flex w-50">
                <div class="crm-content-header-left-item mr-3" style="width:200px">
                    <h5 class="d-inline mr-2 font-weight-bold">{{ $t("message.costs") }}</h5>
            ...
            <div class="crm-content-header-right d-flex w-50 justify-content-end">
                ...
                <div class="crm-content-header-right-item">
                    <export-excel :data="list" :fields="excel_fields" @fetch="controlExcelData()" :worksheet="$t('message.costs')" :name="$t('message.costs')">
                        <el-button size="mini">
        ...
        <table class="table table-bordered table-hover" v-loading="loadingData"
            ...
            <thead>
                <tr>
                    ...
                    <crm-th :column="columns.is_distribution" :sort="sort" @c-change="updateSort"></crm-th>
                ...
                <tr>
                    ...
                    <th v-if="columns.amount.show">
                        <el-input size="mini" v-model="filterForm.amount" :placeholder="columns.amount.title" clearable>
                        </el-input>
                    </th>
                    ...
                    <th v-if="columns.is_distribution.show">
                        <el-select v-model="filterForm.is_distribution" filterable clearable :placeholder="columns.is_distribution.title" size="mini">
            ...
            <transition-group name="flip-list" tag="tbody">
                <tr v-for="cost in list" :key="cost.id">
                    <td v-if="columns.id.show">{{ cost.id }}</td>
                    <td v-if="columns.name.show">{{ cost.name | capitalize }}</td>
                    <td v-if="columns.amount.show">{{ cost.amount | formatNumber }} {{ cost.currency ? cost.currency.symbol : '' }}</td>
                    <td v-if="columns.description.show">{{ cost.description }}</td>
                    <td v-if="columns.is_distribution.show">{{ (cost.is_distribution) ? $t('message.yes') : $t('message.no') }}</td>
                    <td v-if="columns.created_at.show">{{ cost.created_at | dateFormat }}</td>
                    <td v-if="columns.updated_at.show">{{ cost.updated_at | dateFormat }}</td>
                    <td v-if="columns.settings.show" class="settings-td">
                        <crm-setting-dropdown :model="cost" name="costs" :actions="actions" @edit="edit" @delete="destroy">
        ...
        <el-drawer :with-header="false" :visible.sync="drawerUpdate" size="60%" ref="drawerUpdate" @opened="drawerOpened('drawerUpdateChild')"  @closed="drawerClosed('drawerUpdateChild')">
            <crm-update :cost="selectedItem" ref="drawerUpdateChild" drawer="drawerUpdate"></crm-update>
        </el-drawer>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...mapActions({
                updateSort: "costs/updateSort",
                updateFilter: "costs/updateFilter",
                updateColumn: "costs/updateColumn",
                updateList: 'costs/index',
                updatePagination: "costs/updatePagination",
                empty: 'costs/empty',
                delete: 'costs/destroy',
                refreshData: 'costs/refreshData',
            })
        }
    };
</script>
