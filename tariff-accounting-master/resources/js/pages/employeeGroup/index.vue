  ...
  <div class="row table-sm mr-0 ml-0 p-0">
    <div class="crm-content-header d-flex w-100 mb-2">
      <div class="crm-content-header-left d-flex w-50">
        <div class="crm-content-header-left-item mr-3" style="width:220px">
          <h5 class="d-inline mr-2 font-weight-bold">{{ $t("message.employeeGroups") }}</h5>
      ...
      <div class="crm-content-header-right d-flex w-50 justify-content-end">
        ...
        <div class="crm-content-header-right-item">
          <export-excel
            ...
            :worksheet="$t('message.employeeGroups')"
            :name="$t('message.employeeGroups')"
          >
            <el-button size="mini">
    ...
    >
      ...
      <transition-group name="flip-list" tag="tbody">
        <tr v-for="employeeGroup in list" :key="employeeGroup.id" class="cursor-pointer">
          <td v-if="columns.id.show">{{ employeeGroup.id }}</td>
          <td v-if="columns.name.show">{{ employeeGroup.name | capitalize }}</td>
          <td v-if="columns.description.show">{{ employeeGroup.description }}</td>
          <td v-if="columns.created_at.show">{{ employeeGroup.created_at | dateFormat }}</td>
          <td v-if="columns.updated_at.show">{{ employeeGroup.updated_at | dateFormat }}</td>
    ...
    >
      <crm-update :group="selectedItem" ref="drawerUpdateChild" drawer="drawerUpdate"></crm-update>
    </el-drawer>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapGetters({
      list: "employeeGroups/list",
      columns: "employeeGroups/columns",
      pagination: "employeeGroups/pagination",
      filter: "employeeGroups/filter",
      sort: "employeeGroups/sort"
    }),
    actions: function() {
      return ["edit", "delete"];
    }
  ...
  methods: {
    ...mapActions({
      updateList: "employeeGroups/index",
      updateSort: "employeeGroups/updateSort",
      updateFilter: "employeeGroups/updateFilter",
      updateColumn: "employeeGroups/updateColumn",
      updatePagination: "employeeGroups/updatePagination",
      empty: "employeeGroups/empty",
      delete: "employeeGroups/destroy",
      refreshData: "employeeGroups/refreshData"
    })
  }
};
</script>
