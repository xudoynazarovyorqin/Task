<template>
  <div class="bg-white p-2 shadow-sm">
    <div class="d-flex bd-highlight border-bottom pt-2">
      <!-- <div class="flex-grow-1 bd-highlight">{{ $t('message.sale_ready_products')}} : <span class="ml-3"> {{ totalSum | formatMoney }} </span></div> -->
...
export default {
  ...
  methods: {
    ...mapActions({
      editChart: "saleReadyProducts/chart"
    }),
    fetchData() {
        ...
        .then(res => {
          if (res.data.chart_data) {
            ...
            this.dataChart = {
              ...
              datasets: [
                {
                  label: this.$t("message.sale_ready_products"),
