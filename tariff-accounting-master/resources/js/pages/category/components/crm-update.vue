<template>
    <el-col :span="24">
        <header id="el-drawer__title" class="el-drawer__header">
            <span>
                {{ $t('message.edit') }} {{ $t('message.category') | lowerFirst }}
            </span>
            <el-button v-can="['categories.update']" type="primary" size="small" class="mr-1" :loading="waiting" @click="submit()"> {{ $t('message.save_and_exit') }}</el-button>
        ...
        methods:{
            ...
            load(){
                if (!this.loading && this.category) {
                    ...
                    this.getModel(this.category.id)
                    .then(res => {
                        this.form = this.getForm;
                        this.changeLoading()
                    })
