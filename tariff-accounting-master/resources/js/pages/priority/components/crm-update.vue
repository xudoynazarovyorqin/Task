<template>
     <el-col :span="24">
        <header id="el-drawer__title" class="el-drawer__header">
            <span>
                {{ $t('message.edit') }} {{ $t('message.priority') | lowerFirst }}
            </span>
            <el-button v-can="['priority.update']" type="primary" size="small" class="mr-1" :loading="waiting" @click="submit()"> {{ $t('message.save_and_exit') }}</el-button>
...
<script>
 import {mapGetters,mapActions} from 'vuex';
    ...
    export default {
        ...
        methods:{
            ...mapActions({
                save: 'priority/update',
                getModel: 'priority/show'
            }),
            afterOpen() {
            ...
            load(){
                if (!this.loading && this.priority) {
                    this.changeLoading(true);
                    this.getModel(this.priority.id)
                    .then(res =>{
                        this.form = this.getForm;
                        this.changeLoading()
                    })
