<template>
    <el-col :span="24">
        <header id="el-drawer__title" class="el-drawer__header">
            <span>
                {{ $t('message.edit') }} {{ $t('message.currency') | lowerFirst }}
            </span>
            <el-button v-can="['currencies.update']" type="primary" size="small" class="mr-1" :loading="waiting" @click="submit()"> {{ $t('message.save_and_exit') }}</el-button>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...mapActions({
                save: 'currencies/update',
                getModel: 'currencies/show'
            }),
            afterOpen() {
            ...
            load(){
                if (!this.loading && this.currency) {
                    this.changeLoading(true);
                    this.getModel(this.currency.id)
                    .then(res =>{
                        this.form = this.getForm;
                        this.changeLoading()
                    })
