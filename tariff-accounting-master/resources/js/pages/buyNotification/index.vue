<template>
    <div class="row table-sm mr-0 ml-0 p-0">
        ...
        >
            ...
            <thead>
                <tr>
                    ...
                    <crm-th :sort="sort" :column="columns.buy_notificationable_type" @c-change="updateSort"></crm-th>
                    <crm-th :sort="sort" :column="columns.buy_notificationable_id" @c-change="updateSort"></crm-th>
                ...
                <tr>
                    ...
                    <th v-if="columns.buy_notificationable_type.show">
                        <el-select filterable clearable :placeholder="columns.buy_notificationable_type.title" size="mini" v-model="filterForm.buy_notificationable_type">
                            <el-option :label="$t('message.sale')" :value="'sales'"></el-option>
                            <el-option :label="$t('message.assembly')" :value="'assemblies'"></el-option>
                        </el-select>
                    </th>
                    <th v-if="columns.buy_notificationable_id.show">
                        <el-input size="mini" v-model="filterForm.buy_notificationable_id" :placeholder="columns.buy_notificationable_id.title" clearable></el-input>
                    </th>
                    <th v-if="columns.materials.show"></th>
            ...
            <transition-group name="flip-list" tag="tbody">
                <tr v-for="buy_notification in list" :key="buy_notification.id" :class="'cursor-pointer column '+buy_notification.status">
                    ...
                    <td v-if="columns.buy_notificationable_type.show">
                        <template v-if="buy_notification.buy_notificationable_type == 'sales'">Производство</template>
                        <template v-if="buy_notification.buy_notificationable_type == 'assemblies'">Сборка</template>
                    </td>
                    <td v-if="columns.buy_notificationable_id.show">{{ buy_notification.buy_notificationable_id }}</td>
        ...
        <el-dialog :title="$t('message.materials')" :visible.sync="visibleItems" width="50%" top="5vh">
        ...
        <el-drawer :with-header="false" :visible.sync="drawerCreate" size="98%" ref="drawerCreate" @closed="drawerClosed('drawerCreateChild')" @opened="drawerOpened('drawerCreateChild')">
...
<script>
	import create from "@/pages/buy/components/crm-create";
	...
	export default {
		...
		computed: {
			...mapGetters({
				list: "buyNotifications/list",
				columns: "buyNotifications/columns",
				pagination: "buyNotifications/pagination",
				filter: "buyNotifications/filter",
				sort: "buyNotifications/sort"
			})
		},
		methods: {
			...mapActions({
				updateList: "buyNotifications/index",
				updateSort: "buyNotifications/updateSort",
				updateFilter: "buyNotifications/updateFilter",
				updateColumn: "buyNotifications/updateColumn",
				updatePagination: "buyNotifications/updatePagination",
				refreshData: "buyNotifications/refreshData",
				empty: "buyNotifications/empty",
				show: 'buyNotifications/show',
				cancel: "buyNotifications/cancel",
			}),
			showItems(model) {
                ...
                this.show(model.id)
					.then(res => {
						if (res.data && res.data.result) {
							this.items = res.data.result.not_enough_materials;
						}
