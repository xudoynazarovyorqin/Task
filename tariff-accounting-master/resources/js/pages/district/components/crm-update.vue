<template>
  <div>
    <header id="el-drawer__title" class="el-drawer__header">
      <span> {{ $t('message.edit') }} {{ $t('message.district') | lowerFirst }} № {{ district.id }}</span>
      <el-button v-can="['districts.update']" type="success" size="small" class="mr-1" :loading="waiting" @click="submit(false)"> {{ $t('message.save') }}</el-button>
      <el-button v-can="['districts.update']" type="primary" size="small" class="mr-1" :loading="waiting" @click="submit(true)"> {{ $t('message.save_and_exit') }}</el-button>
...
import drawer from '@/utils/mixins/includes/drawer';
  ...
  export default {
      ...
      methods:{
          ...
          load(){
              if (!this.loading && this.district) {
                this.changeLoading(true);
                this.edit(this.district.id)
                  .then(res => {
                      this.form = this.getForm;
                      this.changeLoading();
                  })
