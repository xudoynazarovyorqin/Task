import { index, inventory, getLastId, show, store, update, items, destroy, getStatuses, deleteMaterial, receive, chart, multiDelete, warehouse_materials, print } from '@/api/buys';
...
export const actions = {
    ...
    warehouse_materials({ commit }, params) {
        return new Promise((resolve, reject) => {
            warehouse_materials(params).then(res => {
                commit("SET_WAREHOUSE_MATERIALS", res.data.result.warehouse_materials.data);
                resolve(res.data.result)
            }).catch(err => {
                reject(err.response.data)
            })
