import { index, inventory, show, store, update, destroy, chart } from '@/api/costs';
...
export const actions = {
    ...
    index({ commit }, params = {}) {
        return new Promise((resolve, reject) => {
            index(params).then(res => {
                commit("SET_LIST", res.data.result.data.costs);
    ...
    show({ commit }, id) {
        return new Promise((resolve, reject) => {
            show(id).then(res => {
                commit("SET_MODEL", res.data.result.data.cost);
                resolve(res.data.result)
            }).catch(err => {
                reject(err.response.data)
            })
