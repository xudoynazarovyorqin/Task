import { index, show, store, update, destroy, multiDelete, print, items, getReservations, getDocuments, getLastId } from '@/api/shipments';

export const actions = {
    index({ commit }, params = {}) {
        return new Promise((resolve, reject) => {
            index(params).then(res => {
                commit("SET_LIST", res.data.result.data.shipments);
    ...
    show({ commit }, id) {
        return new Promise((resolve, reject) => {
            show(id).then(res => {
                commit('SET_MODEL', res.data.result.data.shipment);
                resolve(res)
            }).catch(err => {
                reject(err.response.data)
            })
