import { index, inventory, show, store, update, destroy } from '@/api/paymentTypes';
...
export const actions = {
    ...
    index({ commit }, params = {}) {
        return new Promise((resolve, reject) => {
            index(params).then(res => {
                commit("SET_LIST", res.data.result.data.paymentTypes);
    ...
    show({ commit }, id) {
        return new Promise((resolve, reject) => {
            show(id).then(res => {
                commit("SET_MODEL", res.data.result.data.paymentType);
                resolve(res.data.result)
            }).catch(err => {
                reject(err.response.data)
            })
