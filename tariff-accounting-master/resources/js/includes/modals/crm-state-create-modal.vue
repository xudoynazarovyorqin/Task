<template>
    <modal :name="name" width="35%" height="auto" :draggable="true" @before-open="beforeOpen">
        <div class="el-dialog__header cursor-move">
            <span class="el-dialog__title">{{ $t('message.new') }} {{ $t('message.status') | lowerFirst }}</span>
            <button type="button" class="el-dialog__headerbtn" @click="closeModal(name)">
                <i class="el-dialog__close el-icon el-icon-close"></i>
            </button>
        </div>
        <div class="el-dialog__body">
             <el-form ref="form" status-icon :model="form" :rules="rules" label-position="right" label-width="80px" size="small">
...
<script>
    ...
    export default {
        ...
        methods:{
            ...mapActions({
                save: 'states/store',
                updateListInventory: 'states/inventory'
            }),
            beforeOpen(){
                        ...
                        if (valid) {
                            ...
                            this.save(this.form)
                                .then(async (res) => {
                                    ...
                                    this.$emit('crm-close', {created: true, state: res.data.state});
                                    this.$alert(res);
                                    this.changeWaiting();
