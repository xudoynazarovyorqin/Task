<template>
    <modal :name="name" width="40%" height="auto" :draggable="true" @before-open="beforeOpen">
        <div class="el-dialog__header cursor-move">
            <span class="el-dialog__title">{{ $t('message.new') }} {{ $t('message.employeeGroup') | lowerFirst }}</span>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapGetters({
                getForm: 'employeeGroups/form',
                rules: 'employeeGroups/rules',
                columns: 'employeeGroups/columns'
            })
        },
        methods:{
            ...mapActions({
                save: 'employeeGroups/store',
                        ...
                        if (valid) {
                            ...
                            this.save(this.form)
                                .then(async (res) => {
                                    ...
                                    this.$emit('crm-close', {created: true, employeeGroup: res.data.employeeGroup});
                                    this.$alert(res);
                                    this.changeWaiting();
